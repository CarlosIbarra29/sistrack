                        ...
                        <div class="card card-custom">
                            <div class="card-header">
                                <div class="card-title">
                                    ...
                                    <h3 class="card-label">Riesgos tecnológicos ({{ $alcance_id->alcance}})</h3>
                                </div>
                                <div class="card-toolbar">

                                  <a href="{{ route('librotec.listadolibroriesgostec') }}" class="btn btn-light-primary font-weight-bolder mr-3 ml-3">
                                    <i class="la la-arrow-left"></i>Regresar</a>

                                    <!--begin::Button-->
                                    <a href="{{ route('librotec.crearriesgotec', $alcance_id->id) }}" class="btn btn-light-primary font-weight-bolder mr-3 ml-3">
                                        <i class="la la-plus"></i>Nuevo</a>

                                    <a href="{{ route('librotec.riesgotecnologicoidinactivos', $alcance_id->id) }}"
...
{{--                                                 <a  href="" class="btn btn-sm btn-clean btn-hover-icon-success btn-icon mt-1" data-id="{{ $unid->id }}" data-nombre="{{ $unid->factores_riesgo }}" data-toggle="tooltip" data-theme="dark" title="Ver '{{ $unid->factores_riesgo }}'" ><i class="flaticon-eye"></i></a> --}}
                                                ...
                                                <a  href="{{ route('librotec.editarriesgotecnologico', [$alcance_id->id,$unid->id]) }}" class="btn btn-sm btn-clean btn-hover-icon-success btn-icon mt-1" data-id="{{ $unid->id }}" data-nombre="{{ $unid->factores_riesgo }}" data-toggle="tooltip" data-theme="dark" title="Editar '{{ $unid->factores_riesgo }}'" ><i class="flaticon-edit"></i></a>
  ...
  <form method="post" id="tecnologico_delete_form" action="{{ route('librotec.deleteriesgotecnologico') }}" enctype="multipart/form-data">
    @csrf
